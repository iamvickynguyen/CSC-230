	...
	heartbeat:
		...
		if (BUTTON_COUNT < 10)
			while(BUTTON_COUNT != 0)
				inc position to put char (while load char)
				dec BUTTON_COUNT
			if (BUTTON_LENGTH > 1)
				if (BUTTON_LENGTH < 20)
					store dot
		...
		*/
		...
		lds r16, BUTTON_COUNT +1
		cpi r16, 11
		brsh display_dot_dash ; branch if same or higher	

		lds r17, BUTTON_LENGTH
		cpi r17, 2 ; if length <= 1: do nothing
		brlo display_dot_dash
...
timer4:
	...
	*/
	...
	;inc button_length
	lds r19, BUTTON_LENGTH
	inc r19
	sts BUTTON_LENGTH, r19 
